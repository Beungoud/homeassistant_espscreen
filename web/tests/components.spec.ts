...
describe("TileInspector: a live picture on a camera tile (app 0.2.91)", () => {
  ...
  it("lets a live camera on a taller tile fill it, whole or cut, with its name or without, and keeps no defaults (app 0.3.9)", async () => {
    ...
    expect(row(drawer, "Display").find("small").text()).toMatch(/firmware 0\.3\.4/);
    expect(row(drawer, "Display").find("small").classes()).toContain("warn");
    ...
    Object.assign(state.inventory.screens[0], { firmware: "0.3.4" });
