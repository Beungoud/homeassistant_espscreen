...
// a camera has no large value (app 0.3.9). What Home Assistant says an entity can do narrows it further.
...
// A live camera on a 1x2 or 2x2 tile fills the card (app 0.3.9, firmware 0.3.4): whole or cut to fill it, its name on it or nothing.
...
const displayWarns = computed(() => !(display.value === "live" && (pictureCard.value ? supports(0, 3, 4) : supports(0, 2, 77))) && !(display.value === "cover" && supports(0, 2, 78)));
...
const displayHint = computed(() => {
  ...
  if (display.value === "live" && taller.value) return t(supports(0, 3, 4) ? "editor.tile.display.live_card_hint" : "editor.tile.display.live_card_needs_firmware");
