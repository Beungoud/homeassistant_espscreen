...
def demo_layout():
    return validate_layout({'title': 'Demo kaarten', 'tiles': [
        {'entity': 'screen.clock', 'name': '', 'options': {'display': 'analog', 'size': 'wide'}},
...
def messages(inbox, rotate=0, digital=False):
...
async def main():
    ...
    p.add_argument('--digital', action='store_true', help='Digitale klok op een enkele tegel in plaats van analoog dubbelbreed')
    args = p.parse_args()
    ...
    try:
        ...
        for message in messages(inbox.object_id if hasattr(inbox, 'object_id') else 'text.inbox', args.rotate % 9, args.digital):
