...
## The home screen scrolls

The wallpaper, the room name and the clock sit on the page itself and stay put.
...
- LVGL derives the scroll length from the tile positions, so there is no
  content height to maintain by hand.
- Tapping and scrolling do not conflict: once a drag passes LVGL's scroll
  limit (10 px) the short click *and* the long press are cancelled.
- `scroll_snap_y: START` makes the container click into place per row. Set it
  to `NONE` on the `tile_scroll` object for free scrolling.
...
| `TILE*_ACTIVE_STATE` | Optional. Exact Home Assistant state that counts as "active". Leave empty for the default behavior (`on`, or always-on for scenes/scripts). Used by tile 8, where the state is `Vol` / `Leeg` rather than `on` / `off`. |
...
| `TILE*_TAP_PARAM_KEY` | `""` | Extra parameter key for the service call. Supported keys: `preset_mode`, `brightness_pct`, `percentage`, `option`. Any other value is ignored and the service is called with `entity_id` only. |
...
3. **Sensors** — under `sensor:` / `text_sensor:`. `ha_state_tile9` is the only
   ...
   `tile9_cover_position` + `tile9_cover_state` (cover),
   `tile9_climate_*` (climate), `tile9_supported_color_modes` (color light).
   Every subscription costs heap, so skip what the tile does not read.
4. **Widget** — an `obj` under `tile_scroll`'s `widgets:`, at
   `x: ${GRID_COL1_X}` / `y: ${GRID_ROW5_Y}`. Pass `NAN` (floats) or `""`
   (strings) to `open_value_overlay` for any sensor you skipped in step 3.
5. **Ack script** — a `tile9_ack` copy under `script:`.
6. **Refresh lambda** — three lines in `ui_refresh`: a `t9_on` line next to the
   others, a `set_value_auto(id(t9_value), …)` call, and an `apply_tile(t9_on, …)`
   call. Climate tiles also need a `sync_open_climate` line.

Watch `sensor.*_heap_free` in Home Assistant after flashing. This board has no
PSRAM, so free heap is the real ceiling on how many tiles fit.
