# Two buttons and button colours on an alert, 0.3.10 acceptance (firmware 0.3.5)
...
- New tests: `test_alert_overlay.cpp` checks the second button's text and colours and where two buttons stand on every
  ...
  profiles. `test_alert_broadcast.py` covers the event: `show_alert_choice` for a screen with firmware 0.3.5, `show_alert`
