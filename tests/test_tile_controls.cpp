...
int main() {
  ...
  Tile curtain = make("cover.gordijnen", "open", 15); curtain.device_class = "curtain"; curtain.position = 100; curtain.controls = "buttons";
  ...
  assert(status_text(radio) == "Inactief");

  // Climate: mode keys in priority order, at most three, the active one checked.
  Tile ac = make("climate.airco", "cool"); ac.hvac_modes = "[\"off\",\"heat_cool\",\"cool\",\"heat\",\"fan_only\",\"dry\"]"; ac.controls = "mode"; ac.current = 21.5f; ac.target = 20; ac.step = 1;
  ...
  assert(status_text(ac) == "Koelen · 21.5°");
  ac.hvac_action = "cooling"; assert(status_text(ac) == "Koelt · 21.5°");
  ...
  Tile number = make("number.doel", "55"); number.minimum = 0; number.maximum = 100; number.step = 5;
  ...
  Tile timer = make("timer.eieren", "active"); timer.controls = "buttons";
  ...
  assert(key_action(make("scene.avond", "unknown"), RUN).service == "scene.turn_on");
  assert(key_action(make("script.alles", "off"), RUN).service == "script.turn_on");
  assert(key_action(make("input_button.bel", "unknown"), RUN).service == "input_button.press");
  assert(!strcmp(run_label("scene"), "Activeren") && !strcmp(run_label("button"), "Indrukken"));
  assert(key_action(make("switch.bureau", "on"), TOGGLE).service == "switch.turn_off");
