...
class Broadcast(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_two_buttons_go_to_screens_that_draw_them(self):
        """A second button and button colours (firmware 0.3.5+): show_alert_choice where the screen has it, show_alert with
        one button on an older one, and each button's own action once."""
        with tempfile.TemporaryDirectory() as tmp:
            ha = fake_ha()
            ha.states['sensor.d1_fw'] = {'state': '0.3.5'}
            m = Manager(ha, Path(tmp) / 'screens.json')
            with self.assertLogs('screen_manager', 'INFO') as logs:
            ...
            self.assertTrue(any('Hall show one button (the second button needs firmware 0.3.5)' in line for line in logs.output), logs.output)
            ha.calls.clear()
