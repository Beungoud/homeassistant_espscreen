...
PROFILES = [ROOT / 'home-like-2432s028.yaml', ROOT / 'guition-4848s040.yaml', ROOT / 'packages/cyd.yaml', ROOT / 'packages/guition.yaml']
...
ALLOWED_IN_FIRMWARE = {
    ...
    'runtime_tiles.h': ('state_color=lv_color_to_u32(lv_color_hsv_to_rgb(t.hue%360,t.saturation,100))&0xFFFFFF;',),
}
...
class OnePlace(unittest.TestCase):
    ...
    def test_no_profile_writes_a_colour_of_its_own(self):
        for path in PROFILES:
            ...
            self.assertEqual(set(wheel), COLOUR_WHEEL, path.name)
            everything = HEX.findall(text)
            self.assertEqual(sorted(everything), sorted(wheel), f'{path.name}: only the colour wheel keeps fixed hues')
            self.assertNotIn('BG_TOP_COLOR', text)
            self.assertNotIn('ACCENT_AUTO', text)
            self.assertNotRegex(text, r'^\s+(bg|text|border|bg_grad|line|outline|shadow)_color:', path.name)
...
class Paints(unittest.TestCase):
    def test_every_paint_is_defined_adopted_and_used_in_every_profile(self):
        ...
        for path in PROFILES:
            ...
            self.assertEqual(len(adopted), len(explicit) + len(looped), f'{path.name}: every style is filled once')
            self.assertEqual(text.count('theme::fill('), len(explicit) + 1, f'{path.name}: styles are filled in theme::paints only')
            self.assertEqual(set(defined), used, f'{path.name}: every paint style is used, every used one defined')
            for style in defined + ['style_page', 'style_tile', 'style_icon_circle', 'style_room', 'style_time', 'style_title',
                ...
                self.assertIn(style, adopted, f'{path.name}: {style} takes its colours from theme.h')
                self.assertIn(adopted[style], paints, f'{path.name}: {style}')
            for style, paint in adopted.items():
                if style.startswith('paint_'):
                    self.assertEqual(style, f'paint_{paint}', f'{path.name}: a paint style is named after its paint')
            # The styles that carry colours have none written down.
            styles = re.search(r'^  style_definitions:\n(.*?)^  # ---------- ALERT', text, re.M | re.S)[1]
            self.assertNotRegex(styles, r'_color:', path.name)
    ...
    def test_the_look_is_applied_at_boot_after_a_change_and_drawn_again(self):
        for path in PROFILES:
            ...
            self.assertIn('theme::paints();\n          theme::set_dark(settings_screen::dark_mode != 0);', boot,
                          f'{path.name}: the paints filled, then the saved look, before the first frame')
            apply = text.split('  - id: apply_screen_settings\n', 1)[1].split('\n  - id: ', 1)[0]
            self.assertIn('theme::set_dark(settings_screen::dark_mode != 0);', apply, f'{path.name}: a change of the setting')
            ...
            for needle in ('runtime_tiles::restyle();', 'light_controls::restyle();', 'settings_screen::restyle();',
                ...
                self.assertIn(needle, redraw, f'{path.name}: {needle}')
