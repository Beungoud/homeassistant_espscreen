...
class DashboardInbox : public text::Text, public Component {
 public:
  void setup() override { this->publish_state("Klaar voor tegelconfiguratie"); }
 protected:
  runtime_tiles::Chunks chunks_;
  void control(const std::string &value) override {
    if (!chunks_.accept(value, millis())) { this->publish_state("Fout: onvolledig bericht"); return; }
    ...
    chunks_.reset();
    if (length == 0 || length > 4096) { this->publish_state("Fout: ongeldige codering"); return; }
