...
inline void prompt(const char *message = "Houd elke tik even vast") {
  lv_obj_set_pos(cross, TARGETS[point].x-12, TARGETS[point].y-12);
  lv_label_set_text_fmt(instructions, "Tik op het kruisje\nPunt %u/5 - tik %u/3", point+1, sample+1);
...
inline void release(int raw_x, int raw_y) {
  ...
  pressed = false;
  if (esphome::millis()-down_at < 80 || raw_x < 0 || raw_x > 4095 || raw_y < 0 || raw_y > 4095) { prompt("Tik rustig, iets langer"); return; }
  ...
  if (xs[2]-xs[0]>180 || ys[2]-ys[0]>180) { sample=0; prompt("Tik 3x op hetzelfde kruisje"); return; }
  point++; sample=0;
  ...
  if (!fit(raw,next)) { point=0;prompt("Meting afgekeurd; probeer opnieuw");return; }
  if (!preference.save(&next) || !esphome::global_preferences->sync()) { point=0;prompt("Opslaan mislukt; probeer opnieuw");return; }
  calibration=next;
