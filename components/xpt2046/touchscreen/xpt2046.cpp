...
void XPT2046Component::update_touches() {
  ...
  if (touch) {
    ...
    if (this->filter_.sample(true, x_raw, y_raw) == TouchFilter::POSITION) {
      this->add_raw_touch_position_(0,
          this->correction_.x(this->filter_.x(), this->filter_.y()),
          this->correction_.y(this->filter_.x(), this->filter_.y()), z_raw);
    } else {
      this->skip_update_ = true;
    }
  } else if (this->filter_.sample(false, 0, 0) == TouchFilter::HOLD) {
    this->skip_update_ = true;
  }
}
