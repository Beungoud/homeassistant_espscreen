...
class XPT2046Component : public Touchscreen,
 ...
 public:
  void set_raw_correction(float xx, float xy, float xc, float yx, float yy, float yc) {
    correction_.set(xx, xy, xc, yx, yy, yc);
  }
  int16_t filtered_raw_x() const { return filter_.x(); }
  int16_t filtered_raw_y() const { return filter_.y(); }
